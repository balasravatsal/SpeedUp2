...
class AutofillExecutor(
    ...
    /**
     * Re-find the field on screen and fill it (used after overlay is hidden).
     */
    fun fillField(packageName: String, fingerprint: FieldFingerprint, value: String): Boolean {
        ...
        Log.w(TAG, "fillField: node not found for ${fingerprint.canonical} label='${fingerprint.label}'")
        return false
    }
    ...
    private fun fillNodeLive(node: AccessibilityNodeInfo, value: String): Boolean {
        // Tap to focus (critical for Chrome WebView fields)
        tapNode(node)
        ...
        // Second tap + paste for stubborn WebView inputs
        tapNode(node)
        Thread.sleep(ACTION_DELAY_MS)
        if (tryPasteFromClipboard(node, value)) {
        ...
        Log.w(TAG, "Failed to fill (actions=${node.actionList.map { it.id }}, editable=${node.isEditable})")
        return false
    }
    ...
    private fun tryPasteFromClipboard(node: AccessibilityNodeInfo, value: String): Boolean {
        ...
        node.performAction(AccessibilityNodeInfo.ACTION_FOCUS)
        Thread.sleep(ACTION_DELAY_MS)
        // Select all existing content so paste replaces it instead of appending
    ...
    private fun findNodeByFingerprint(
        ...
        collectEditables(root, packageName, editables)
        ...
        pickFirst { node ->
            val hint = node.hintText?.toString()?.trim().orEmpty()
            fp.hint.isNotBlank() && hint.contains(fp.hint, ignoreCase = true)
        }?.let { return it }

        pickFirst { node ->
            val label = FieldMapper.findLabelFor(node)
            fp.label.isNotBlank() && label.contains(fp.label, ignoreCase = true)
        }?.let { return it }
        ...
        pickFirst { node ->
            ...
            val match = FieldMapper.matchField(
                id, hint, node.contentDescription?.toString().orEmpty(), label, node.inputType, packageName
            )
    ...
    private fun collectEditables(
        ...
        for (i in 0 until node.childCount) {
            ...
            collectEditables(child, packageName, out)
            child?.recycle()
        }
    }
